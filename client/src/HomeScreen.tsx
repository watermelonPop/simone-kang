...
function HomeScreen({ accessToken, isMobile, handleUnlock, themes, currentTheme, setCurrentTheme }: HomeScreenProps){
    ...
    useEffect(() => {
        ...
        const validPages = ['Spotify', 'Weather', 'DayNight', 'Contact', 'Portfolio', 'AboutMe', 'Settings', 'Resume', 'Github', 'Linkedin'];
        const match = validPages.find(page => path.toLowerCase().startsWith(page.toLowerCase()));
        if (match) {
            handleUnlock();
            // Use the full path as the modal page so "Weather/today" is preserved
            openModal(path, window.location.pathname);
        }
    }, []);
    ...
    useEffect(() => {
        if (!currentSong || !accessToken) return;
        ...
        fetchLikedStatus();
    }, [currentSong?.id, accessToken]); // only re-run when the song ID changes, not every re-render
    ...
    const openModal = (page: string, fullPath?: string) => {
        navigate(fullPath ?? `/${page}`);
        setBottomSlidingOut(true);
        setTimeout(() => {
            setBottomHidden(true);
            setModalPage(page);
            setModalOpen(true);
            setBottomSlidingOut(false);
        }, 250);
    };
        ...
        <div className="homeScreenOuter">
            ...
            <div className={`homeBottomOuter ${bottomSlidingOut ? 'slideDownOut' : ''} ${bottomSlidingIn ? 'slideUpIn' : ''} ${!showBackdrop && isPlaying === true && currentSong !== null && !bottomHidden ? 'songPlaying' : ''}`}>
                {!showBackdrop && isPlaying === true && currentSong !== null && !bottomHidden ? (
                    <>
                    <div className='homeBottomOuterClamp'>
